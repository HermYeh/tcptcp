...
impl Protocol {
    ...
    pub fn connect(dest: &str) -> io::Result<Self> {
    ...
    pub fn filetransfer(&mut self) -> std::io::Result<()> {
        ...
        let name = CString::new("test.jpg")
            .expect("CString::new failed")
            .into_bytes();
        let mut buf_name = [0; 255];
        name.iter().enumerate().for_each(|(i, b)| {
            buf_name[i] = *b;
        });
        //read file into stream and send it through the socket
        let mut readfile=File::open("E:\\test.jpg")?;
